                ...
                ({ id, title, pages, authorsNames }: Book) =>
                  isFilterFoundInProperties(title, pages, id, authorsNames) && (
                    <tr key={id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-1 py-4">{Number.isInteger(pages) ? `${pages} pages` : "Not informed"}</td>
                      <td className="px-1 py-4">{<span className="block max-w-52 truncate">{authorsNames}</span>}</td>
                    </tr>
                  ),
              )}
          </tbody>
